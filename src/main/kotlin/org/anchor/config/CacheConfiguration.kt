...
class CacheConfiguration(
    ...
    fun jcacheConfiguration(jHipsterProperties: JHipsterProperties): javax.cache.configuration.Configuration<Any, Any> {
        ...
        jcacheConfig.setExpiryPolicyFactory(
            CreatedExpiryPolicy.factoryOf(
                Duration(
                    TimeUnit.SECONDS,
                    jHipsterProperties.cache.redis.expiration.toLong()
                )
            )
        )
    ...
    fun cacheManagerCustomizer(jcacheConfiguration: javax.cache.configuration.Configuration<Any, Any>): JCacheManagerCustomizer {
        return JCacheManagerCustomizer {
            createCache(it, org.anchor.repository.UserRepository.USERS_BY_LOGIN_CACHE, jcacheConfiguration)
            createCache(it, org.anchor.repository.UserRepository.USERS_BY_EMAIL_CACHE, jcacheConfiguration)
            // jhipster-needle-redis-add-entry
        }
    }

    private fun createCache(
        cm: javax.cache.CacheManager,
        cacheName: String,
        jcacheConfiguration: javax.cache.configuration.Configuration<Any, Any>
    ) {
