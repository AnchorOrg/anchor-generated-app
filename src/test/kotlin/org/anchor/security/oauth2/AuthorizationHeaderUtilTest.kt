...
class AuthorizationHeaderUtilTest {
    ...
    @InjectMocks
    private lateinit var authorizationHeaderUtil: AuthorizationHeaderUtil
    ...
    fun setup() {
        ...
        doReturn(restTemplateBuilder).`when`(restTemplateBuilder)
            .additionalMessageConverters(any(HttpMessageConverter::class.java))
    ...
    fun getAuthorizationHeader_JwtAuthentication() {
        val jwtToken = JwtAuthenticationToken(
            Jwt(
                ...
                hashMapOf("alg" to "HS256") as Map<String, Any>?,
                hashMapOf("sub" to SUB_VALUE) as Map<String, Any>?
            )
        )
    ...
    fun getAuthorizationHeader_OAuth2Authentication_InvalidClient() {
        ...
        doReturn(authorizedClient).`when`(clientService)
            .loadAuthorizedClient<OAuth2AuthorizedClient>(eq(VALID_REGISTRATION_ID), eq(SUB_VALUE))

        Assertions.assertThatThrownBy {
            authorizationHeaderUtil.getAuthorizationHeader()
        }.isInstanceOf(OAuth2AuthorizationException::class.java)
    ...
    fun getAuthorizationHeader_OAuth2Authentication_RefreshToken() {
        ...
        doReturn(refreshResponse).`when`(restTemplate)
            .exchange(any(RequestEntity::class.java), eq(OAuthIdpTokenResponseDTO::class.java))
    ...
    fun getAuthorizationHeader_OAuth2Authentication_RefreshTokenFails() {
        ...
        val restTemplate = mock(RestTemplate::class.java)
        doThrow(OAuth2AuthorizationException(OAuth2Error("E"), "error")).`when`(restTemplate)
            .exchange(any(RequestEntity::class.java), eq(OAuthIdpTokenResponseDTO::class.java))
        doReturn(restTemplate).`when`(restTemplateBuilder).build()

        Assertions.assertThatThrownBy { authorizationHeaderUtil.getAuthorizationHeader() }
    ...
    private fun getTestOAuth2AuthorizedClient(accessTokenExpired: Boolean): OAuth2AuthorizedClient {
        ...
        if (accessTokenExpired) {
            ...
            try {
                ...
                fail("Error in Thread.sleep(1) : " + e.message)
            }
        } else {
